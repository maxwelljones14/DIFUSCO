...
class DjikstraGraphDataset(torch.utils.data.Dataset):
  ...
  def get_example(self, idx):
    ...
    path = path.split(' ')
    path = np.array([[int(path[i]), int(path[i + 1])] for i in range(0, len(path) - 1)])

    return points, path

  def __getitem__(self, idx):
    points, path = self.get_example(idx)
    ...
    #   print(f"index: {idx}")
      ...
      path_edges = torch.from_numpy(path)
      path_edges_empty = torch.zeros(idx_knn.shape)
      path_edges_empty -= 1
      for i in range(path_edges.shape[0]):
        found = False
        for j in range(path_edges_empty.shape[1]):
            if path_edges[i][1] == idx_knn[path_edges[i][0]][j]:
                path_edges_empty[path_edges[i][0]][j] = path_edges[i][1]
                found = True
            elif path_edges[i][0] == idx_knn[path_edges[i][1]][j]:
                path_edges_empty[path_edges[i][1]][j] = path_edges[i][0]
                found = True
        if not found:
            print("Error: Edge not found in KNN graph")
            print(path_edges[i])
            print(idx_knn[path_edges[i][0]])
            print(path_edges_empty[path_edges[i][0]])
            print(idx_knn[path_edges[i][1]])
            exit(1)
    #   print(path_edges_empty)
    #   print(edge_index_1)
      path_edges = path_edges_empty.reshape(-1)
    #   path_edges = path_edges.reshape((-1, 1)).repeat(1, sparse_factor).reshape(-1)
      path_edges = torch.eq(edge_index_1, path_edges).reshape(-1, 1)
      graph_data = GraphData(x=torch.from_numpy(points).float(),
                             edge_index=edge_index,
                             edge_attr=path_edges)
      ...
      path = np.pad(path, ((0, points.shape[0] - path.shape[0]), (0, 0)), 'constant', constant_values=-1)
      return (
          torch.LongTensor(np.array([idx], dtype=np.int64)),
          graph_data,
          torch.from_numpy(point_indicator).long(),
          torch.from_numpy(edge_indicator).long(),
          torch.from_numpy(path).long(),
      )
